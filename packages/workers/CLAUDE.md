...
- The `raw_content` poller (`poller.ts`) runs hourly by default. Do not shorten it in production: every poll queries Neon and resets its scale-to-zero timer, which is fixed at 5 minutes on the Free plan (not adjustable — only paid plans can change it). Any DB access more frequent than every 5 min keeps the compute awake around the clock and exhausts the monthly compute-hour allowance. Use `RAW_CONTENT_POLL_INTERVAL_MS` for a faster local loop.
