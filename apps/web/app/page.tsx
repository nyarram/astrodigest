...
import { ArrowUpRight, ExternalLink, Telescope } from 'lucide-react'
import { useLatestDigest } from '@/lib/queries'
import { StreamingSummary } from '@/components/StreamingSummary'
import { cn } from '@/lib/utils'
import type { QuickHit, SourceType, SpaceNewsItem } from '@astrodigest/shared'
...
const SOURCE_LABELS: Record<SourceType, string> = {
  ...
  planetary: 'Planetary Society',
}

// ─── Shared small components ─────────────────────────────────────────────────

function SourceBadge({ source }: { source: SourceType }): JSX.Element {
  return (
...
export default function HomePage(): JSX.Element {
  ...
  if (isError || digest === undefined) return <ErrorState />
      ...
      <section className="mb-12">
        ...
        <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-muted">
          <Image
            ...
            // unoptimized because image domains (NASA, ESO) aren't yet in next.config.ts
            // remotePatterns — remove once those are configured
            unoptimized
          />
        </div>
        <div className="mt-3">
